...
const View = ({backgroundColor, changeBkColor, color, dark, opacity, setSide, side, imgOp, setPopOver, popOver,delay, setDelay,nextStage, close})=>{
    ...
    const darkMode="#3c3c3c";
    const ligthMode="#cfcfcf";
return(
    <CardContainer  bkColor={backgroundColor} color={color} onClick={changeBkColor} style={side?{ transform: "scaleX(-1)"}:{}}>

        

        <Card opacity={close?"0;":"1;"} shadow={dark?"#666":"#333"} bkColor={dark?darkMode:ligthMode} hoverBkColor={dark?"#414141":"#d5d5d5"} style={side?{ transform: "scaleX(-1)"}:{}}>
            {!side?<></>:<BackButton bkColor={dark?darkMode:ligthMode}
            ...
            logos.map((e, i)=>{
                ...
                {!side?
                <></>:
                <Info shadow={dark?darkMode:ligthMode} position={"absolute;"} marginTop={"25vh;"}><Span>this is the beggining</Span>...</Info> 
                }
            ...
            bkColor={dark?darkMode:ligthMode}
            color={color}
            opacity={opacity}
